...
class Process():
    def __init__(self):
        ...
        self.routes = self.trips.LINEID.unique()
    ...
    def make_weather(self):
        ...
        important_weather_features = ['dt', 'temp', 'pressure', 'humidity', 'wind_speed', 'weather_main']
        return weather[important_weather_features] 

    def create_df(self, bus_line, direction):
        print(bus_line, direction)
        start_time = time.time()
        trip = self.trips.loc[(self.trips['LINEID'] == bus_line) & (self.trips['DIRECTION'] == direction)]
        chunk_size = 1000000
        ...
        for count, leave_chunk in enumerate(pd.read_csv('D:\data\\clean_leavetimes.csv', chunksize=chunk_size)):
            print(count)
            out = pd.merge(trip , leave_chunk, on = ['TRIPID', 'DAYOFSERVICE'], how = 'inner')
            temp.append(out)
            if count == 3:
                break
        ...
        dfObj['COMB'] = dfObj['DAYOFSERVICE'].map(str) + dfObj['TRIPID'].map(str)
        u = dfObj['COMB'].unique()
    ...
    def create_result_df(self, dfObj, u):
        ...
        for count,date_trip in enumerate(u):
            day_trip = dfObj.loc[(dfObj['COMB'] == date_trip)].sort_values(by=['PROGRNUMBER'])
            if (len(day_trip['PROGRNUMBER']) > max_progrnum): #only full trips
                ...
                try:
                    ...
                    time_be = day_trip['ACTUALTIME_ARR_y'].shift(-1) - day_trip['ACTUALTIME_ARR_y']
                    ...
                    hour = day_trip['ACTUALTIME_ARR_y'].apply(lambda x: x//3600)
                    ...
                    rush_binary = day_trip['ACTUALTIME_ARR_y'].apply(self.rush_hour)
                    ...
                    # Merge these columns
                    a = pd.concat([tripid, datetime, dayofweek, month, day, hour, rush_binary, day_trip['PROGRNUMBER'], end_point, time_be], axis = 1, sort=False)

                    # Change the name of columns
                    a.columns = ['tripid', 'dt','dayofweek','month','day','hour', 'rush_hour','progrnum','stop_id','cum_duration']
