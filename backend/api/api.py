    ...
    def get(self):
        ...
        for i in range(len(model_info)):
            for j in range(len(model_info[i])):
                ...
                print(t_stop)
                prediction = get_prediction(start["start_time"], int(midnight.timestamp(
                ))+start["start_time"], t_stop["duration"], leg["route"], leg["direction"])
                ...
                bus_travel["duration"] = t_stop["duration_p"] - \
                    start["duration_p"]
                bus_travel["transit"]["arr"]["time"] = midnight.timestamp() + \
                    t_stop["dep_p"]
                bus_travel["transit"]["dep"]["time"] = midnight.timestamp() + \
                    start["dep_p"]
                bus_travel["transit"]["source"] = "TFI HOOLIGANS"
                bus_travel["transit"]["arr"]["delta"] = t_stop["dep_p"] - \
                    t_stop["dep_time"]
                bus_travel["transit"]["dep"]["delta"] = start["dep_p"] - \
                    start["dep_time"]
    ...
    def get(self):
        ...
        for i in range(len(model_input)):
            ...
            for j in range(len(model_input[i])):
                ...
                bus_travel["duration"] = t_stop["duration_p"] - \
                    start["duration_p"]
                bus_travel["transit"]["arr"]["time"] = int(
                    midnight.timestamp())+t_stop["dep_p"]
                bus_travel["transit"]["dep"]["time"] = int(
                    midnight.timestamp())+start["dep_p"]
                bus_travel["transit"]["source"] = "TFI HOOLIGANS"
                bus_travel["transit"]["arr"]["delta"] = t_stop["dep_p"] - \
                    t_stop["dep_time"]
                bus_travel["transit"]["dep"]["delta"] = start["dep_p"] - \
...
def get_prediction(timestamp, datetimestamp, duration, route_id, direction):
    ...
    model_input = model_input_create(
        humidity, duration, weather, wd, m, h, holiday, feature_set)
